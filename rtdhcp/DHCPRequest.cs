...
namespace DotNetProjects.DhcpServer
{
    ...
    public class DHCPRequest
    {
        ...
        internal DHCPRequest(byte[] data, UdpClient udpClient, DHCPServer server)
        {
            dhcpServer = server;
        ...
        public Dictionary<DHCPOption, byte[]> GetAllOptions()
        {
            ...
            Util.log("options length:" + requestData.options.Length);
            //string str = BitConverter.ToString(requestData.options).Replace("-", " ");
            //Util.log(str);

            for (int i = 0; i < requestData.options.Length; i++)
            {
                DDataID = (DHCPOption)requestData.options[i];
                if (DDataID == DHCPOption.Pad_Option) continue; //padding
                if (DDataID == DHCPOption.END_Option) break;
                DataLength = requestData.options[i + 1];
                byte[] dumpData = new byte[DataLength];
                Array.Copy(requestData.options, i + 2, dumpData, 0, DataLength);
                Util.log(String.Format("{0} {1} {2}", requestData.options[i].ToString("X"), requestData.options[i + 1].ToString("X"), BitConverter.ToString(dumpData).Replace("-", " ")));
